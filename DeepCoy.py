...
h:      GNN hidden size
j:      noise/augmentation vector size
'''
...
class DenseGGNNChemModel(ChemModel):
    ...
    def default_params(cls):
        ...
        params.update({
                        ...
                        "output_name": '', # Filename of generated SMILES. If '' then name set based on run ID.
                        "subgraph_freq_file": '', # Location of pkl containing subgraph frequencies. Not necessary for molecule generation.
                        "use_subgraph_freqs": True, #  False: Vanilla cross entropy, True: Reweigh using subgraph frequencies
                        })
    ...
    def prepare_specific_graph_model(self) -> None:
        ...
        # The weights for calculating mean and variance
        ...
        self.weights['variance_combine_weights_in'] = tf.Variable(glorot_init([out_dim, h_dim]))
        ...
        self.weights['node_symbol_weights_in'] = tf.Variable(glorot_init([h_dim, self.params['num_symbols']]))
        ...
        for direc in ["in", "out"]:
            for i in range(self.num_edge_types):
                self.weights['edge_type_%d_%s' % (i, direc)] = tf.Variable(glorot_init([feature_dimension, feature_dimension]))
                self.weights['edge_type_biases_%d_%s' % (i, direc)] = tf.Variable(np.zeros([1, feature_dimension]).astype(np.float32)) 
                self.weights['edge_type_output_%d_%s' % (i, direc)] = tf.Variable(glorot_init([feature_dimension, 1]))
            # weights for generating edge logits
            self.weights['edge_iteration_'+direc] = tf.Variable(glorot_init([feature_dimension, feature_dimension]))
            self.weights['edge_iteration_biases_'+direc] = tf.Variable(np.zeros([1, feature_dimension]).astype(np.float32)) 
            self.weights['edge_iteration_output_'+direc] = tf.Variable(glorot_init([feature_dimension, 1]))            
    ...
    def compute_mean_and_logvariance(self):
        ...
        # AVERAGE ENCODING - output molecule
        ...
        # PER VERTEX ENCODING - input molecule
        reshped_last_h = tf.reshape(self.ops['final_node_representations_in'], [-1, h_dim])
    ...
    def sample_with_mean_and_logvariance(self):
        ...
        z_prior = tf.reshape(self.placeholders['z_prior'], [-1, out_dim])
        z_prior_in = tf.reshape(self.placeholders['z_prior_in'], [-1, h_dim])
        ...
        mean = mean * self.ops['graph_state_mask_in']
        roll_shift = tf.cast(tf.reduce_min(tf.reduce_sum(self.ops['graph_state_mask_in'],1)), tf.int32) # Shift by smallest number of atoms in smallest mol
        inverted_mask = tf.ones(tf.shape(self.ops['graph_state_mask_in'])) - self.ops['graph_state_mask_in']
        update_vals = self.placeholders['z_prior_in'] * inverted_mask # Extra nodes from N(0,1)
        mean = tf.reshape(tf.add(mean, update_vals), [-1, h_dim]) # Fill in extra vertices with random noise from N(0,1)
        
        # Combine input molecule node embeddings with output molecule embedding
    ...
    def generate_cross_entropy(self, idx, cross_entropy_losses, edge_predictions, edge_type_predictions):
        ...
        subgraph_freqs = self.placeholders['new_compound_freqs_'+direc][:, idx, :] # [b, v]
        subgraph_edge_freqs = self.placeholders['new_compound_freqs_edge_'+direc][:,idx,:,:] # [b, e, v]
        ...
        # Reweigh edge probabilities by subgraph frequencies
        if self.params['use_subgraph_freqs']:
            subgraph_freqs_mask = self.placeholders['edge_masks_'+direc][:, idx, :]
            subgraph_freqs = tf.concat([subgraph_freqs,
                                        tf.reduce_sum(subgraph_freqs*subgraph_freqs_mask+SMALL_NUMBER, axis=1, keepdims=True) \
                                         / tf.reduce_sum(subgraph_freqs_mask+SMALL_NUMBER, axis=1, keepdims=True)], axis=1) # [b, v+1] # add stop node multiplier
            edge_softmax = tf.nn.softmax(edge_logits) * subgraph_freqs # reweight probabilities by subgraph occurrence
            edge_softmax = edge_softmax / tf.reduce_sum(edge_softmax, axis=1, keepdims=True) # Normalise

            # Reweight edge loss by subgraph frequencies
            subgraph_freqs_mask = tf.concat([subgraph_freqs_mask, np.ones((self.params['batch_size'], 1))], axis=1) # [b, v+1]
            edge_loss_multi = tf.tile(tf.reduce_sum(subgraph_freqs*subgraph_freqs_mask+SMALL_NUMBER, axis=1, keepdims=True) \
            ...
            edge_type_probs = edge_type_probs / tf.reduce_sum(edge_type_probs, axis=1, keepdims=True) # Normalise
    ...
    def construct_logit_matrices(self):
        ...
        latent_node_state= self.get_node_embedding_state(self.placeholders["latent_node_symbols_"+out_direc]) 
        ...
        self.ops['node_symbol_logits_'+in_direc]=tf.reshape(tf.matmul(tf.reshape(self.ops['z_sampled_'+in_direc],[-1, h_dim]), self.weights['node_symbol_weights_'+in_direc]) + 
    ...
    def construct_loss(self):
        ...
        # KL loss
        # Node embeddings of input molecule
        ...
        # Node embeddings of output molecule
            ...
            for list_idx, starting_idx in enumerate(list_of_starting_idx):
                ...
                if self.params["sample_transition"] and list_idx > 0:
                    incremental_results[res_idx][-1]=[x+y for x, y in zip(incremental_results[res_idx][-1], [incremental_adj_mat,distance_to_others,
                                            ...
                                            new_compound_frequencies, new_compound_frequencies_edge])]
                else:
    ...
    def process_raw_graphs(self, raw_data, is_training_data, file_name, bucket_sizes=None):
        ...
        for d, incremental_result_1 in zip(raw_data, incremental_results[1]):
            ...
            bucketed[chosen_bucket_idx].append({
                ...
                'new_compound_freqs_out': incremental_result_1[9],
                'new_compound_freqs_edge_out': incremental_result_1[10],
    ...
    def make_batch(self, elements, maximum_vertice_num):
        ...
        batch_data = {'adj_mat_in': [], 'adj_mat_out': [], 'init_in': [], 'init_out': [], 'edge_type_masks_out':[],
                'edge_type_labels_out':[], 'edge_masks_out':[], 'edge_labels_out':[],
                'node_mask_in': [], 'node_mask_out': [], 'node_sequence_out':[],
                ...
                'new_compound_freqs_out': [], 'new_compound_freqs_edge_out': []}
        for d in elements: 
            ...
            # sparse to dense for saving memory           
                ...
                new_compound_freqs = transition_freqs_to_dense(d['new_compound_freqs'+direc], maximum_vertice_num) 
                new_compound_freqs_edge = transition_freqs_edge_to_dense(d['new_compound_freqs_edge'+direc], maximum_vertice_num, num_edge_types=self.num_edge_types) 
                ...
                batch_data['overlapped_edge_features'+direc].append(overlapped_edge_features + 
                    [np.zeros((maximum_vertice_num)) 
                                for _ in range(max_iteration_num-d['number_iteration'+direc])])
                batch_data['new_compound_freqs'+direc].append(new_compound_freqs + 
                    [np.ones((maximum_vertice_num))
                                for _ in range(max_iteration_num-d['number_iteration'+direc])])
                batch_data['new_compound_freqs_edge'+direc].append(new_compound_freqs_edge +
                    [np.ones((self.num_edge_types, maximum_vertice_num))
                                for _ in range(max_iteration_num-d['number_iteration'+direc])])
    ...
    def get_dynamic_feed_dict(self, elements, latent_node_symbol, incre_adj_mat, num_vertices, 
                    distance_to_others, overlapped_edge_dense, new_compound_freqs_dense, new_compound_freqs_edge_dense, node_sequence, edge_type_masks, edge_masks, 
        ...
        if incre_adj_mat is None:
            ...
            new_compound_freqs_dense=np.ones((1,1,1))
            new_compound_freqs_edge_dense=np.ones((1,1,1,1))
        ...
        return {
                ...
                self.placeholders['new_compound_freqs_out']: new_compound_freqs_dense,
                self.placeholders['new_compound_freqs_edge_out']: new_compound_freqs_edge_dense,
    ...
    def search_and_generate_molecule(self, initial_idx, valences, 
        ...
        while len(queue) > 0:
            ...
            while True:
                ...
                new_compound_freqs_dense = np.ones((1, max_n_vertices)) # [1, v]
                new_compound_freqs_edge_dense = np.ones((1, self.num_edge_types, max_n_vertices)) # [1, e, v]
                ...
                feed_dict=self.get_dynamic_feed_dict(elements, [sampled_node_symbol_one_hot],
                            [incre_adj_mat], max_n_vertices, [distance_to_others], [overlapped_edge_dense], [new_compound_freqs_dense], [new_compound_freqs_edge_dense],
    ...
    def generate_graph_with_state(self, random_normal_states, random_normal_states_in, num_vertices,
        ...
        node_symbol_batch_feed_dict=self.get_dynamic_feed_dict(elements, None, None,
                                     num_vertices, None, None, None, None, None, None, None,
                                     random_normal_states, random_normal_states_in)
        ...
        generated_all_smiles.append(elements['smiles_in'] + " " + Chem.MolToSmiles(best_mol))
    ...
    def compensate_node_length(self, elements, bucket_size):
        ...
        real_length_out = get_graph_length([elements['mask_in']])[0]+self.params["compensate_num"]
    ...
    def make_minibatch_iterator(self, data, is_training: bool):
        ...
        for step in range(len(bucket_at_step)):
            ...
            batch_feed_dict = {
                ...
                self.placeholders['new_compound_freqs_out']: batch_data['new_compound_freqs_out'],
                self.placeholders['new_compound_freqs_edge_out']: batch_data['new_compound_freqs_edge_out'],
            }
            bucket_counters[bucket] += 1
